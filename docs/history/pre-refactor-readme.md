...
> **历史快照（2026-08-23，文档重构前）**：本文保留旧 README 中混合记录的阶段说明、命令和当时状态，不再承担当前文档职责。请以[当前状态](../status.md)和[运行手册](../runbook.md)为准。

> **后续状态注记（2026-08-28）**：本文记录的第七、八阶段通用细分范围、提示词版本和续跑方式已经失效，不得作为当前操作说明。当前实现只对`同人`进行作品或系列 IP 分类，并只为特殊一级类别生成本地辅助分组；当前完整结果确有 15,134 条，但来自新的选择性范围批次，不能据此把本文中的旧设计视为已执行。
...
脚本默认使用 OpenAI SDK 兼容接口 `https://provider.example/v1`（历史地址已脱敏）、模型 `v4 flash`、每批 30 张、最多 1300 次 HTTP 调用和 4096 输出 token。服务端会拒绝别名 `flash`，其 `/v1/models` 为该 API 密钥返回的实际模型 ID 是 `v4 flash`。每次请求的首条消息都会以 `<已脱敏的旧版提示词前缀>` 开头。先预检最新的完整扫描批次、分类标准和调用量，不发送模型请求，也不需要密钥：
...
### 第七阶段：超过 100 张的一级目录二次分类

第七阶段读取完整的 052 分类结果及其扫描索引，以 PNG 和 JSON 文件合计数为口径，对文件数严格大于 100 的一级目录增加一个二级类别。同一卡片内容的不同封面只请求模型一次并复用结果；只增加一层，不递归拆分仍超过 100 张的二级目录。

- `同人`按作品或系列 IP 归档，并规范化原神、绝区零、崩坏系列等常见别名。
- 其他普通类别按题材、关系、场景或玩法归档，无法稳定判断时使用`其他`。
- `排除`和`未分类`根据已有复核原因使用固定目录，`标准外`沿用原模型类别；这些项目不再次上传正文。
- 成功结果立即写入检查点，续跑会绑定分类结果、扫描索引、模型配置和提示词版本。

只读预检会创建可续跑批次，但不会发送模型请求：

```powershell
node .\src\070-refine-character-cards.mjs `
  .\reports\classification-reviews-052\20260822T121236978Z-f3d98577 `
  .\reports\scans\20260821T062149Z `
  --dry-run
```

确认预检后，仅在当前 PowerShell 进程提供密钥并续跑预检生成的批次：

```powershell
$env:MODEL_API_KEY = '你的 Sub Key'
node .\src\070-refine-character-cards.mjs --resume=".\reports\refinements\<批次>"
Remove-Item Env:MODEL_API_KEY
```

结果位于`reports/refinements/<批次>/`：`checkpoint.jsonl`用于续跑，`refinements.jsonl`是 15,134 条完整文件结果，`taxonomy.json`汇总二级目录及数量，另有`review.csv`、`usage.jsonl`、错误文件、`summary.json`和`run.json`。

### 第八阶段：生成完整二次分类树

第八阶段默认从`data/已分类角色卡`读取一级分类副本，生成复制到`data/二次分类角色卡`的预览计划。超过阈值的目录采用`一级/二级/文件`，未超过阈值的目录保持`一级/文件`；新目录最终仍应包含完整 15,134 张卡。

```powershell
node .\src\080-organize-refined-character-cards.mjs `
  .\reports\refinements\<完整批次>
```

计划写入`reports/refinement-plans/<批次>/`。目标目录必须不存在或为空；检查`plan.csv`和`summary.json`后，把同批次`approval.json`的`approved`改为`true`再执行：

```powershell
node .\src\080-organize-refined-character-cards.mjs --execute `
  .\reports\refinement-plans\<批次>
```

第八阶段只允许复制，执行前后核验 SHA-256，拒绝覆盖；同名不同内容使用短哈希后缀。现有`未分类角色卡`和`已分类角色卡`均保持不变。
...
- **所处阶段**：第六阶段已把权威 052 结果完整复制到`data/已分类角色卡`；第七、八阶段代码已实现，第七阶段真实预检已完成，等待提供模型密钥续跑，尚未创建或复制`data/二次分类角色卡`。
- **完整交接**：第五阶段来源与风险见 [05阶段结果交接.md](./phase-05-handoff.md)；二次分类实施状态和续跑命令见 [07阶段结果交接.md](./phase-07-08-handoff.md)。
- **具体备注**：
  ...
  - 052 最终结果包含 12,754 条正式分类、2,075 条明确排除和 305 条仍需人工判断；二次分类保留这些一级处置结论，仅在其下增加原因目录。
  - 第六阶段批次`20260822T131941624Z-0ab16690`的 15,134 条复制计划已全部执行成功，无失败记录。
  - 第七阶段预检批次`20260823T150516348Z-9069445a`准确选中 24 个超阈值目录；2,551 条特殊目录文件确定性处理，12,298 个普通类别唯一内容预计首轮 419 次模型请求。预检没有发送请求。
