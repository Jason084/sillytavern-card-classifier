...
const defaultReportsDirectory = join(root, 'reports', 'classification-reviews');
const promptVersion = 'classification-review-v1-aibh-batch10';
...
if (resumeArgument === '') throw new Error('--resume 必须指定已有 051 复核批次目录');
...
if (resumeArgument) {
  ...
  if (sourceRun.status !== 'complete') throw new Error('051 只能复核已完整结束的第五阶段分类批次');
...
const targetRows = sourceRows.filter((row) => row.needs_review === true);
const targetHashes = new Set(targetRows.map((row) => String(row.card_sha256 ?? '')).filter(Boolean));
if (!targetHashes.size) throw new Error('首次分类结果中没有 needs_review=true 的项目');
...
if (dryRun) {
  console.log(JSON.stringify({
    ...
    run_command: `node .\\src\\051-classify-character-cards.mjs --resume="${batchDirectory}"`,
  }, null, 2));
  process.exit(0);
}
...
async function classifyBatch(batch, batchId) {
  ...
  for (let round = 1; round <= 3 && unresolved.length; round += 1) {
    try {
      ...
      ], { budget, onEvent, phase: 'classification-review', requestId: `${batchId}-round-${round}` });
...
if (workerError) {
  await writeFile(join(batchDirectory, 'fatal-error.json'), JSON.stringify({
    ...
    resume_command: `node .\\src\\051-classify-character-cards.mjs --resume="${batchDirectory}"`,
  }, null, 2), 'utf8');
...
await reviewHandle.write(csv(['relative_path', 'name', 'first_decision', 'first_category', 'recheck_decision', 'recheck_category', 'recheck_reason', 'needs_review', 'human_category', 'review_notes']));
...
const fullCounts = new Map(); const transitionCounts = new Map(); let remainingNeedsReview = 0;
for (const source of sourceRows) {
  const cardHash = String(source.card_sha256 ?? ''); let result = source;
  if (source.needs_review === true) {
    const firstPass = { decision: source.model_decision, category: source.category };
    ...
    if (decision) {
      result = {
        ...
        first_pass_category: firstPass.category,
    ...
    await reviewHandle.write(csv([result.relative_path, result.name, firstPass.decision, firstPass.category, result.model_decision, result.category, result.recheck_reason, result.needs_review, '', '']));
    const transition = `${firstPass.decision}->${result.model_decision}`;
...
console.log(`051 二次复核结果已生成：${batchDirectory}`);
