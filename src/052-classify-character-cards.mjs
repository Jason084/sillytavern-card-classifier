#!/usr/bin/env node
/*
 * 第五阶段再次复核：复制 051 的断点续跑机制，只把 051 仍返回 review 的唯一内容交给模型第三次判断。
...
const sourceReportsDirectory = join(root, 'reports', 'classification-reviews');
const defaultReportsDirectory = join(root, 'reports', 'classification-reviews-052');
const promptVersion = 'classification-review-v2-aibh-batch2';
...
if (resumeArgument === '') throw new Error('--resume 必须指定已有 052 复核批次目录');
...
if (resumeArgument) {
  ...
  if (sourceRun.status !== 'complete') throw new Error('052 只能复核已完整结束的 051 分类复核批次');
...
const targetRows = sourceRows.filter((row) => row.needs_review === true && row.model_decision === 'review' && row.recheck_status === 'complete');
const targetHashes = new Set(targetRows.map((row) => String(row.card_sha256 ?? '')).filter(Boolean));
if (!targetHashes.size) throw new Error('051 结果中没有仍为 review 的已完成项目');
...
if (dryRun) {
  console.log(JSON.stringify({
    ...
    run_command: `node .\\src\\052-classify-character-cards.mjs --resume="${batchDirectory}"`,
  }, null, 2));
  process.exit(0);
}
...
async function classifyBatch(batch, batchId) {
  let unresolved = batch; let lastError = null; let splitOnFailure = false;
    ...
    try {
      ...
      const response = await requestModelJson(modelSettings, [
        ...
        { role: 'user', content: JSON.stringify({
          ...
          allowed_categories: allowedCategories,
          cards: unresolved.map((group) => ({
            ...group.model_input,
            previous_pass_decision: group.previous.decision,
            previous_pass_category: group.previous.category,
          })),
        }) },
      ], { budget, onEvent, phase: 'classification-review-2', requestId: `${batchId}-round-${round}` });
...
if (workerError) {
  await writeFile(join(batchDirectory, 'fatal-error.json'), JSON.stringify({
    ...
    resume_command: `node .\\src\\052-classify-character-cards.mjs --resume="${batchDirectory}"`,
  }, null, 2), 'utf8');
...
await reviewHandle.write(csv(['relative_path', 'name', 'original_first_decision', 'previous_decision', 'previous_category', 'previous_reason', 'third_decision', 'third_category', 'third_reason', 'needs_review', 'human_category', 'review_notes']));
...
for (const source of sourceRows) {
  ...
  if (targetHashes.has(cardHash)) {
    ...
    await reviewHandle.write(csv([result.relative_path, result.name, source.first_pass_decision, previousPass.decision, previousPass.category, source.recheck_reason, result.model_decision, result.category, result.recheck_2_reason, result.needs_review, '', '']));
...
const summary = {
  ...
  remaining_model_review_file_records: remainingModelReview,
...
console.log(`052 三次判断结果已生成：${batchDirectory}`);
