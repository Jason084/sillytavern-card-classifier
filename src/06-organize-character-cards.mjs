...
function safeCategory(value) {
  ...
  if (!output || output === '.' || output === '..') throw new Error(`无效分类目录名：${value}`); return output;
}
...
async function previewPlan() {
  ...
  const batchDirectory = join(reportsRoot, runId); await mkdir(batchDirectory, { recursive: true });
  ...
  for await (const line of createInterface({ input: createReadStream(classificationsPath), crlfDelay: Infinity })) {
    ...
    const category = safeCategory(result.category); const initialDestination = join(destinationRoot, category, basename(result.file_name ?? result.relative_path));
...
async function executePlan() {
  ...
  const approval = JSON.parse(await readFile(approvalPath, 'utf8')); const actualPlanHash = await sha256(planPath);
  ...
  const destinationRoot = resolve(approval.destination_root); const logPath = join(batchDirectory, `execution-${runId}.jsonl`);
  const logHandle = await open(logPath, 'w'); const csvHandle = await open(join(batchDirectory, `execution-${runId}.csv`), 'w');
  ...
  for await (const line of createInterface({ input: createReadStream(planPath), crlfDelay: Infinity })) {
    ...
    try {
      ...
      const sourceHash = await sha256(plan.source_path); if (sourceHash !== plan.sha256) throw new Error('来源文件已变化，SHA-256 不匹配');
      ...
      const destinationHash = await sha256(plan.destination_path);
  ...
  await writeFile(join(batchDirectory, `execution-summary-${runId}.json`), JSON.stringify({ plan_run_id: approval.plan_run_id, executed_utc: new Date().toISOString(),
