...
import {
  EXCLUDED_PARENT, FALLBACK_SUBCATEGORY, NONSTANDARD_PARENT, REFINEMENT_SCOPE, REFINEMENT_THRESHOLD, UNRESOLVED_PARENT,
  normalizeSubcategory, parentCategory, parentSpecificInstruction, reasonText, refinementGroupId,
...
const promptVersion = 'refinement-v4-fanwork-ip-filter-aware';
...
const { positionals, resumeArgument, dryRun } = parseModelPhaseArguments(args, '--resume 必须指定已有二次分类批次目录');
...
if (resumeArgument) {
  ...
  const required = [['refinement_scope', REFINEMENT_SCOPE], ['model_version', modelSettings.model], ['prompt_version', promptVersion], ['api_base_url', modelSettings.baseUrl], ['batch_size', modelSettings.batchSize], ['max_attempts', modelSettings.maxAttempts], ['max_output_tokens', modelSettings.maxOutputTokens]];
...
const refinementsHandle = await open(join(batchDirectory, 'refinements.jsonl'), 'w'); const reviewHandle = await open(join(batchDirectory, 'review.csv'), 'w'); const errorsHandle = await open(join(batchDirectory, 'model-errors.jsonl'), 'w'); const indexErrorsHandle = await open(join(batchDirectory, 'index-errors.jsonl'), 'w');
await reviewHandle.write(csv(['relative_path', 'parent_category', 'subcategory', 'selected_for_refinement', 'classification_source', 'used_fallback', 'review_notes']));
...
const subcategoryCounts = new Map(); let fallbackFiles = 0;
for (const context of fileContexts) {
  ...
  if (context.selected) { const key = `${context.parent}\0${refinement.subcategory}`; subcategoryCounts.set(key, (subcategoryCounts.get(key) ?? 0) + 1); }
  const output = { relative_path: context.record.relative_path, file_name: context.record.file_name, sha256: context.record.sha256, card_sha256: context.cardSha256, model_decision: context.record.model_decision, original_category: context.record.category, parent_category: context.parent, selected_for_refinement: context.selected, subcategory: refinement.subcategory, refinement_source: refinement.classification_source, model_input_profile: refinement.model_input_profile ?? null, used_fallback: refinement.used_fallback, model_version: refinement.model_version ?? null, prompt_version: refinement.prompt_version ?? promptVersion, review_reason: context.mode === 'deterministic' ? reasonText(context.record) : null };
  await refinementsHandle.write(`${JSON.stringify(output)}\n`); await reviewHandle.write(csv([output.relative_path, output.parent_category, output.subcategory, output.selected_for_refinement, output.refinement_source, output.used_fallback, '']));
}
await refinementsHandle.close(); await reviewHandle.close(); await errorsHandle.close(); await indexErrorsHandle.close();
const taxonomy = [...selectedParents].sort((a, b) => a.localeCompare(b, 'zh-CN')).map((parent) => ({ parent, file_count: parentCounts.get(parent), subcategories: [...subcategoryCounts].filter(([key]) => key.startsWith(`${parent}\0`)).map(([key, count]) => ({ subcategory: key.slice(parent.length + 1), count })).sort((a, b) => b.count - a.count || a.subcategory.localeCompare(b.subcategory, 'zh-CN')) }));
await writeFile(join(batchDirectory, 'taxonomy.json'), JSON.stringify({ generated_utc: new Date().toISOString(), refinement_scope: REFINEMENT_SCOPE, threshold: REFINEMENT_THRESHOLD, levels_added: 1, parents: taxonomy }, null, 2), 'utf8');
const summary = { run_id: runId, generated_utc: new Date().toISOString(), refinement_scope: REFINEMENT_SCOPE, source_classifications: classificationsPath, source_classifications_sha256: classificationsSha256, source_index: indexPath, source_index_sha256: indexSha256, source_file_records: classificationRecords.length, threshold: REFINEMENT_THRESHOLD, levels_added: 1, parent_counts: [...parentCounts].sort((a, b) => b[1] - a[1]).map(([parent, count]) => ({ parent, count, refinement_mode: refinementMode(parent) })), selected_parents: [...selectedParents], selected_parent_count: selectedParents.size, selected_file_records: fileContexts.filter((item) => item.selected).length, unselected_file_records: fileContexts.filter((item) => !item.selected).length, unique_model_groups: modelGroups.size, unique_completed_from_checkpoint: completed.size, unique_completed_with_metadata: [...completed.values()].filter((item) => item.model_input_profile === 'metadata').length, unique_without_semantic_content: withoutSemantic.size, unique_model_failed_with_fallback: failed.size, fallback_file_records: fallbackFiles, deterministic_file_records: deterministicFiles, http_requests_total: budget.used, http_requests_this_process: budget.usedThisProcess, model_request_batches_this_process: requestBatches, api_base_url: modelSettings.baseUrl, model_version: modelSettings.model, prompt_version: promptVersion, batch_size: modelSettings.batchSize, concurrency: modelSettings.concurrency, max_attempts: modelSettings.maxAttempts, max_output_tokens: modelSettings.maxOutputTokens, unique_failed_or_incomplete: 0, status: 'complete', resume_command: `node .\\src\\070-refine-character-cards.mjs --resume="${batchDirectory}"` };
