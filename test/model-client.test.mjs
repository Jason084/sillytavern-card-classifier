...
test('模型配置由通用环境变量提供，密钥只读取 MODEL_API_KEY', () => {
  ...
  try {
    ...
    const defaults = modelSettingsFromEnv({
      baseUrl: 'https://provider.example/v1', model: 'default-model', batchSize: 30,
      maxOutputTokens: 4_096, requireApiKeyForDefault: true,
    });
    assert.equal(defaults.url, 'https://provider.example/v1/chat/completions');
    ...
    process.env.MODEL_BATCH_SIZE = '7'; process.env.MODEL_MAX_OUTPUT_TOKENS = '999';
